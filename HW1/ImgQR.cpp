...
void write_bmp(unsigned char *header,unsigned char ***data,int offset,int depth,int width,int height,FILE *fp,int regulization)
{
...
int main(void)
{
	...
	for(int ccase=1;ccase<=2;ccase++)
	{
		...
		if(bmpfile.bits_perpixel==32)
		{
			...
			{
				sprintf(arr2,"output1_%d.bmp",shift);
				if((fp=fopen(arr2,"wb"))==0)
				{
					fprintf(stderr,"open file error\n");
					return -1;
				}	
				rewind(fp);
				write_bmp(header,temp,bmpfile.bitmap_dataoffset,4,bmpfile.width,bmpfile.height,fp,1<<(shift+1));
				fclose(fp);
			}
		...
		else if(bmpfile.bits_perpixel==24)
		{
			...
			for(int shift=1;shift<=3;shift++)
			{
				sprintf(arr2,"output2_%d.bmp",shift);
				if((fp=fopen(arr2,"wb"))==0)
				{
					fprintf(stderr,"open file error\n");
					return -1;
				...
				rewind(fp);
				write_bmp(header,temp,bmpfile.bitmap_dataoffset,3,bmpfile.width,bmpfile.height,fp,1<<(shift+1));
				fclose(fp);
			}
		}
